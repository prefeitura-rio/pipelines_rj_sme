...
with source as (
    select * from {{ source('educacao_basica_frequencia_staging', 'CLS_AlunoAvaliacaoTurma') }}
    {% if is_incremental() %}
...
renamed as (
    select
        ...
        SAFE_CAST(REGEXP_REPLACE(TRIM({{ adapter.quote("aat_numeroFaltas") }}), r'\.0$', '') AS STRING) AS aat_numeroFaltas,
        SAFE_CAST(REGEXP_REPLACE(TRIM({{ adapter.quote("aat_numeroAulas") }}), r'\.0$', '') AS STRING) AS aat_numeroAulas,
